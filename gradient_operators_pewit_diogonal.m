...
% Define the Prewitt operator masks (including diagonal edges)
dx = [-1 0 1; -1 0 1; -1 0 1];
dy = [-1 -1 -1; 0 0 0; 1 1 1];
dxy = [0 1 1; -1 0 1; -1 -1 0];

% Compute the gradients in the x and y directions
gradientX = conv2(grayImage, dx, 'same');
gradientY = conv2(grayImage, dy, 'same');
gradientXY = conv2(grayImage, dxy, 'same');

% Compute the magnitude and direction of the gradients
gradientMagnitude = sqrt(gradientX.^2 + gradientY.^2 + gradientXY.^2);
